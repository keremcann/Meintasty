...
namespace Meintasty.Data
{
    public class OrderRepositoryAsync : MeintastyDbConnection, IOrderRepositoryAsync
    {
        ...
        /// <returns></returns>
        public async Task<GeneralResponse<List<Order>>> GetAllByInfoAsync(Order item)
        {
            var data = new GeneralResponse<List<Order>>();
            data.Value = new List<Order>();
            if (!connection.Success)
            ...
            if (item.UserId != default(int) && item.UserId > 0) 
            {
                spName = "sel_OrdersByUserId";
                parameters.Add("@UserId", item.UserId);
            }
            else if (item.RestaurantId != default(int) && item.RestaurantId > 0)
            {
